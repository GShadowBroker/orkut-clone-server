...
module.exports = () => {
  const queries = {
    ...
    getFriendSuggestions: async (root, args, context) => {
      ...
      if (currentUser.Friends.length === 0) {
        const suggestions = await User.findAll({
          ...
          where: {
            id: {
              [Op.not]: [...requestees, currentUser.id],
            },
          },
      ...
      const friendsOfFriendsIds = friends
        ...
        .filter(
          (f) =>
            f.toString() !== currentUser.id.toString() &&
            !friendsIds.includes(f.toString())
        );
    ...
    findPhoto: async (root, args) => {
      ...
      if (!photo)
        throw new UserInputError("Foto não encontrada ou inválida", {
          invalidArgs: args,
        });
    ...
    allCommunities: async (root, args) => {
      ...
      limitComment = limitComment ? limitComment : 1;
      limitTopic = limitTopic ? limitTopic : 1;
    ...
    findCommunityMembers: async (root, args) => {
      ...
      if (filter) {
        members = await community.getMembers({
          ...
          order: [random ? [Sequelize.fn("RANDOM")] : ["name", "ASC"]],
          limit,
          offset,
        });
      } else {
